...
begin
  ...
  { intro hconv,
    ...
    rw convergence_space.Inf_iff at hconv,
    let q := convergence_space.final ps (λ _, α) α (λ p, p.val) (λ _, id),
    have hub : ∀ p ∈ ps, p ≤ q := λ p hmem f x hconv,
      or.inr ⟨⟨p, hmem⟩, f, x, hconv, by simp, rfl⟩,
    rcases (hconv q hub) with hle | hexists,
    { exact or.inl hle },
    { obtain ⟨p, g : filter α, y, ⟨hg, hle', heq⟩⟩ := hexists,
      simp at heq, simp at hle',
      rw heq at hg,
      change converges_ p.val g x at hg,
      refine or.inr ⟨p.val, p.property, _⟩,
      exact le_converges_ p.val hle' hg }},
  { intros hor, cases hor,
    { exact le_converges_ (Sup ps) hor (pure_converges_ (Sup ps) x) },
    { obtain ⟨p, hmem, hconv⟩ := hor,
